...
//    private static final float MOUNTAINS2_V = 1.185F;
	private static TerrainPopulator makeMountains(Terrain terrainType, @Deprecated Seed seed, Noise ground, TerrainSettings.Terrain settings, float horizontalScale, float verticalScale, boolean makeFancy) {
		int scaleH = /*Math.round(MOUNTAINS_H);*/Math.round(410.0F * settings.horizontalScale);
		...
		return TerrainPopulator.make(terrainType, ground, Noises.mul(height, 0.7F /*MOUNTAINS_V*/ * verticalScale), Erosion.LEVEL_1.source(), Noises.min(Noises.mul(height, Noises.constant(-1.0F)), Noises.constant(-0.08F)), settings);
	}
	
	public static TerrainPopulator makeMountains(@Deprecated Seed seed, Noise ground, TerrainSettings.Terrain settings, float horizontalScale, float verticalScale, boolean makeFancy) { 
		return makeMountains(TerrainType.MOUNTAINS_1, seed, ground, settings, horizontalScale, verticalScale, makeFancy);
	}
	
	public static TerrainPopulator makeMountainChain(@Deprecated Seed seed, Noise ground, TerrainSettings.Terrain settings, float horizontalScale, float verticalScale, boolean makeFancy) { 
		return makeMountains(TerrainType.MOUNTAIN_CHAIN, seed, ground, settings, horizontalScale, verticalScale, makeFancy);
	}
    ...
    public static TerrainPopulator makeMountains3(@Deprecated Seed seed, Noise ground, TerrainSettings.Terrain settings, float verticalScale, boolean makeFancy) {
    	Noise cell = Noises.worleyEdge(seed.next(),400/*MOUNTAINS2_H*/ , EdgeFunction.DISTANCE_2, DistanceFunction.EUCLIDEAN);
    	cell = Noises.mul(cell, 1.2F);
    	cell = Noises.clamp(cell, 0.0F, 1.0F);
    	cell = Noises.warpPerlin(cell, seed.next(), 200, 2, 100.0F);
		...
		height = Noises.cache2d(height);
		return TerrainPopulator.make(TerrainType.MOUNTAINS_3, ground, Noises.mul(height, 0.645F/*MOUNTAINS2_V*/ * verticalScale), Erosion.LEVEL_1.source(), Noises.min(Noises.mul(height, Noises.constant(-1.0F)), Noises.constant(-0.08F)), settings);
    }
