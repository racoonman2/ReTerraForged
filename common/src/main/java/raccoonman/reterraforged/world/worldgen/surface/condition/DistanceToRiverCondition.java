...
class DistanceToRiverCondition extends ThresholdCondition {
	...
	public DistanceToRiverCondition(Context context, float threshold, Noise variance) {
	...
	public record Source(float threshold, Holder<Noise> variance) implements SurfaceRules.ConditionSource {
		...
		public DistanceToRiverCondition apply(Context ctx) {
			return new DistanceToRiverCondition(ctx, this.threshold, this.variance.value());
		}
