...
class DistanceToRiverCondition extends ThresholdCondition {
	
	public DistanceToRiverCondition(Context context, Noise threshold, Noise variance) {
		super(context, threshold, variance);
	}

	@Override
	protected float sample(Cell cell) {
		return 1.0F - cell.riverMask;
	}
	...
	public record Source(Holder<Noise> threshold, Holder<Noise> variance) implements SurfaceRules.ConditionSource {
		...
		@Override
		public DistanceToRiverCondition apply(Context ctx) {
			return new DistanceToRiverCondition(ctx, this.threshold.value(), this.variance.value());
		}
