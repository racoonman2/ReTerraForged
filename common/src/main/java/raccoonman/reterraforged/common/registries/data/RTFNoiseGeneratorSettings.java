...
public final class RTFNoiseGeneratorSettings {
	...
	public static void register(BootstapContext<NoiseGeneratorSettings> ctx) {
		...
		ctx.register(DEFAULT, createTFSettings(ctx, densityFunctions, noiseParams));
    }
    ...
    private static NoiseGeneratorSettings createTFSettings(BootstapContext<NoiseGeneratorSettings> ctx, HolderGetter<DensityFunction> densityFunctions, HolderGetter<NormalNoise.NoiseParameters> noiseParams) {
...
//	  			overworldRouter.initialDensityWithoutJaggedness()
	  			new DensityFunctions.HolderHolder(densityFunctions.getOrThrow(RTFDensityFunctions.INITIAL_DENSITY_WITHOUT_JAGGEDNESS)),
