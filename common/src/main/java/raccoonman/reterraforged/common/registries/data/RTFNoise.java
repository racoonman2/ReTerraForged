...
public final class RTFNoise {
	...
	public static final ResourceKey<Noise> Y_SCALE = resolve("terrain/y_scale");
	public static final ResourceKey<Noise> TERRAIN_HEIGHT = resolve("terrain/terrain_height");
    ...
    public static void register(BootstapContext<Noise> ctx) {
    	...
    	Noise base = register(ctx, BASE, createBase(baseFrequency, baseWarp, scale));
    	
    	Noise continentWarp = register(ctx, CONTINENT_WARP, createContinentWarp());
    	Noise coastMax = register(ctx, COAST_MAX, Source.constant(0.3F));
    	Noise shoreMax = register(ctx, SHORE_MAX, Source.constant(-0.19F));
    	Noise oceanMax = register(ctx, OCEAN_MAX, Source.constant(-0.35F));
    	Noise continent = register(ctx, CONTINENT, createContinent(baseFrequency, continentWarp, scale));
    	...
    	Noise ocean = register(ctx, OCEAN, createOcean());
    	...
    	Noise yScale = register(ctx, Y_SCALE, Source.constant(512.0D));
    	
    	register(ctx, TERRAIN_HEIGHT, terrain.mul(yScale));
    }
...
//    		);
    ...
    private static Noise createContinent(Noise baseFrequency, Noise warp, int scale) {
...
//    			new Falloff.Point(-0.5F, -1.1F, 0.1F)
    	...
    	).freq(1.0F / scale, 1.0F / scale).warp(warp.unique(), warp.unique(), Source.constant(0.2D)).freq(baseFrequency, baseFrequency).clamp(-3.8F, 3.8F);
    }
    ...
    private static Noise createBase(Noise baseFrequency, Noise warp, int scale) {
    	final float threshold = 0.525F;
        return new MapToRange(new SampleAtNearestCell(CellSource.PERLIN.freq(1.0F / 5.0F, 1.0F / 5.0F).shift(656), 1.0F, 0.75F, 2, CellShape.SQUARE).freq(1.0F / scale, 1.0F / scale).warp(warp.unique(), warp.unique(), Source.constant(0.2D)), threshold + 0.01F, threshold + 0.25F).freq(baseFrequency, baseFrequency).mul(Source.constant(0.25F));
    }
    ...
    private static Noise createOcean() {
        return Source.simplex(64, 3).shift(8763214).scale(0.4F).mul(Source.constant(0.078F)).add(Source.constant(0.0449F));
    }
...
//    	return new ContinentLerp(continent, land, ocean, MIN_HEIGHT, -0.35F, -0.19F, 0.3F);
    }
