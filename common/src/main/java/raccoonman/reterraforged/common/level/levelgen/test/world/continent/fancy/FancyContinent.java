...
public class FancyContinent implements Noise, RiverGenerator {
	...
	public FancyContinent(int seed, int nodes, float radius, GeneratorContext context, FancyContinentGenerator continent) {
		ControlPoints controlPoints = ControlPoints.make(context.settings.world().controlPoints);
	...
	public long getValueId(float x, float y) {
		...
		return NoiseUtil.packMix(id, value);
	}

	@Override
	public Rivermap generateRivers(int x, int z, long id, int seed) {
		return this.riverGenerator.generateRivers(x, z, id, seed);
	}
