...
import raccoonman.reterraforged.common.level.levelgen.test.world.rivermap.river.RiverConfig;
...
public class FancyRiverGenerator extends BaseRiverGenerator<FancyContinentGenerator> {
	...
	public Rivermap generateRivers(int x, int z, long id, int seed) {
		...
		for (Island island : ((FancyContinentGenerator) this.continent).getSource().getIslands()) {
			this.generateRoots(((FancyContinentGenerator) this.continent).getSource(), island, random, warp, roots, seed);
	...
	private void generateRoots(FancyContinent continent, Island island, Random random, GenWarp warp, List<Network.Builder> roots, int seed) {
		...
		for (int i = 0; i < segments.length; ++i) {
			...
			this.collectSegmentRoots(continent, island, segment, riverCount, random, warp, roots, riverCount);
		}
		Segment first = segments[0];
		this.collectPointRoots(continent, island, first.a, first.scaleA, endCount, random, warp, roots, endCount);
		Segment last = segments[segments.length - 1];
		this.collectPointRoots(continent, island, last.b, last.scaleB, endCount, random, warp, roots, endCount);
	}

	private void collectSegmentRoots(FancyContinent continent, Island island, Segment segment, int count, Random random, GenWarp warp, List<Network.Builder> roots, int seed) {
		float dx = segment.dx;
		float dy = segment.dy;
		...
		for (int i = 0; i < count; ++i) {
			...
			if (scale != 0.0f) {
				...
				this.addRoot(x1, y1, x2, y2, this.main, random, warp, roots, seed);
			}
		}
	}

	private void collectPointRoots(FancyContinent continent, Island island, Vec2f vec, float radiusScale, int count, Random random, GenWarp warp, List<Network.Builder> roots, int seed) {
		...
		for (int i = 0; i < count; ++i) {
			...
			if (scale != 0.0F) {
				...
				if (continent.compute(endX, endZ, seed) <= 0.1F) {
					this.addRoot(startX, startZ, endX, endZ, this.main, random, warp, roots, seed);
				}
			}
		}
	}

	private void addRoot(float x1, float z1, float x2, float z2, RiverConfig config, Random random, GenWarp warp, List<Network.Builder> roots, int seed) {
		...
		roots.add(network);
		this.generateForks(network, River.FORK_SPACING, this.fork, random, warp, roots, 0, seed);
	...
	private static float getExtendScale(int islandId, float startX, float startZ, float dx, float dz, float radius, FancyContinent continent) {
		...
		for (int i = 0; i < 25; ++i) {
			...
			if (NoiseUtil.unpackLeft(packed) != islandId) {
				return 0.0F;
			}
			if (NoiseUtil.unpackRightf(packed) < 0.1F) {
				return scale;
			}
			scale += 0.075F;
		}
