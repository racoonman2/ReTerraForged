...
public record StrataRule(ResourceLocation name, Holder<Noise> selector, List<Strata> strata, int iterations) implements SurfaceRules.RuleSource {
	...
	public Source apply(Context ctx) {
		if(ctx.system instanceof SurfaceSystemExtension systemExt && (Object) ctx.randomState instanceof RandomStateExtension randomExt) {
			return new Source(ctx, randomExt.shift(this.selector.value()), systemExt.getOrCreateStrata(this.name, this::generateStrata));
		} else {
		...
		public List<Layer> generateLayers(RandomSource random) {
	        ...
	        for (int i = 0; i < layers; i++) {
	            ...
	            if (index != lastIndex) {
	                ...
	                result.add(new Layer(material, this.noise.value().shift(random.nextInt()).scale(depth), seed));
	            }
	        }
	        return result;
		}
