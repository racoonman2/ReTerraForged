...
public class NoiseTree {
	...
	public static class PointList<V, T extends Point<V>> {
		...
		public static <V, T extends Point<V>> Codec<PointList<V, T>> codec(Codec<List<T>> points, int paramCount) {
			return ExtraCodecs.nonEmptyList(points).xmap((values) -> new NoiseTree.PointList<>(paramCount, values), NoiseTree.PointList::values);
		}
	}
