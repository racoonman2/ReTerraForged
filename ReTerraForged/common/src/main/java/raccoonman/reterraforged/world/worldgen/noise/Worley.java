...
record Worley(float frequency, float distance, CellFunction cellFunction, DistanceFunction distanceFunction, float min, float max) implements Noise {
	...
	public Worley(float frequency, float distance, CellFunction cellFunction, DistanceFunction distanceFunction) {
		this(frequency, distance, cellFunction, distanceFunction, min(cellFunction), max(cellFunction));
	}
	...
	public float compute(float x, float z, int seed) {
        x *= this.frequency;
        z *= this.frequency;
        float value = sample(x, z, seed, this.distance, this.cellFunction, this.distanceFunction);
	...
	public Noise mapAll(Visitor visitor) {
		return visitor.apply(new Worley(this.frequency, this.distance, this.cellFunction, this.distanceFunction));
	}
    ...
    public static float sample(float x, float y, int seed, float distance, CellFunction cellFunction, DistanceFunction distanceFunction) {
        ...
        return cellFunction.apply(seed, cellX, cellY, nearest, vec2f);
    }
    
    private static float min(CellFunction function) {
        return -1.0F;
    }
    
    private static float max(CellFunction function) {
