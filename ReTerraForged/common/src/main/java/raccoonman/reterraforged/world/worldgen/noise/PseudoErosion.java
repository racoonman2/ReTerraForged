...
public record PseudoErosion(Noise input, int seed, int octaves, float strength, float gridSize, float amplitude, float lacunarity, float distanceFallOff, BlendMode blendMode, ThreadLocal<float[]> cache) implements Noise {
	public static final MapCodec<PseudoErosion> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
		Noise.HOLDER_HELPER_CODEC.fieldOf("input").forGetter(PseudoErosion::input),
		Codec.INT.fieldOf("seed").forGetter(PseudoErosion::seed),
		Codec.INT.fieldOf("octaves").forGetter(PseudoErosion::octaves),
		Codec.FLOAT.fieldOf("strength").forGetter(PseudoErosion::strength),
		Codec.FLOAT.fieldOf("grid_size").forGetter(PseudoErosion::gridSize),
		Codec.FLOAT.fieldOf("amplitude").forGetter(PseudoErosion::amplitude),
		Codec.FLOAT.fieldOf("lacunarity").forGetter(PseudoErosion::lacunarity),
		Codec.FLOAT.fieldOf("distance_falloff").forGetter(PseudoErosion::distanceFallOff),
		BlendMode.CODEC.fieldOf("blend_mode").forGetter(PseudoErosion::blendMode)
	).apply(instance, PseudoErosion::new));

	public PseudoErosion(Noise input, int seed, int octaves, float strength, float gridSize, float amplitude, float lacunarity, float distanceFallOff, BlendMode blendMode) {
	...
	public Noise mapAll(Visitor visitor) {
		return new PseudoErosion(this.input.mapAll(visitor), this.seed, this.octaves, this.strength, this.gridSize, this.amplitude, this.lacunarity, this.distanceFallOff, this.blendMode);
	}

	@Override
	public MapCodec<PseudoErosion> codec() {
		return CODEC;
	}

	@Override
	public boolean equals(Object o) {
		return o instanceof PseudoErosion other && other.input.equals(this.input) && other.seed == this.seed && other.octaves == this.octaves && other.strength == this.strength && other.gridSize == this.gridSize && other.amplitude == this.amplitude && other.lacunarity == this.lacunarity && other.distanceFallOff == this.distanceFallOff && other.blendMode.equals(this.blendMode);
	}
