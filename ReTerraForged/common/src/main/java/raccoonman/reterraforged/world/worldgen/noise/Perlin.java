...
record Perlin(float frequency, int octaves, float lacunarity, float gain, Interpolation interpolation, float min, float max) implements Noise {
	public static final MapCodec<Perlin> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
		...
		Interpolation.CODEC.forGetter(Perlin::interpolation)
    ...
    public Perlin(float frequency, int octaves, float lacunarity, float gain, Interpolation interpolation) {
	...
	public float compute(float x, float z, int seed) {
		...
		for(int i = 0; i < this.octaves; i++) {
			sum += sample(x, z, i + seed, this.interpolation) * amplitude;
			x *= this.lacunarity;
			z *= this.lacunarity;
			amplitude *= this.gain;
		}
