...
public record Simplex(float frequency, int octaves, float lacunarity, float gain, float min, float max) implements Noise {
    ...
    public Simplex(float frequency, int octaves, float lacunarity, float gain) {
    	this(frequency, octaves, lacunarity, gain, -max(octaves, gain), max(octaves, gain));
    }
    ...
    public static float sample(float x, float y, int seed) {
        return sample(x, y, seed, 79.869484F);
    }
