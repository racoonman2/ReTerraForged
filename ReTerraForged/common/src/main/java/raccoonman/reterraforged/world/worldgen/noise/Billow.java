...
record Billow(float frequency, int octaves, float lacunarity, float gain, float[] spectralWeights, float min, float max) implements Noise {
	...
	public Billow(float frequency, int octaves, float lacunarity, float gain) {
		this(frequency, octaves, lacunarity, gain, calculateSpectralWeights(octaves, lacunarity));
	}
	
	private Billow(float frequency, int octaves, float lacunarity, float gain, float[] spectralWeights) {
		this(frequency, octaves, lacunarity, gain, spectralWeights, 0.0F, calculateMaxBound(spectralWeights, gain));
	}
	...
	public float compute(float x, float z, int seed) {
        ...
        for (int octave = 0; octave < this.octaves; octave++) {
            float signal = Perlin.sample(x, z, seed + octave, Interpolation.CURVE3);
	...
	public boolean equals(Object o) {
		return o instanceof Billow other && other.frequency == this.frequency && other.octaves == this.octaves && other.lacunarity == this.lacunarity && other.gain == this.gain;
	}
